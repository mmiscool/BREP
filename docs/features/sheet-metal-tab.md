...
## Behaviour
- Tabs reuse the `BREP.Sweep` translator to form solids and automatically remove consumed sketches.
- Every resulting solid receives `userData.sheetMetal` metadata (`baseType: TAB`, `thickness`, `bendRadius`, etc.) plus mirrored entries in the `MetadataManager`.
