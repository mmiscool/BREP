...
- `newFeature()` allocates the declarative entry object `{ type, inputParams, persistentData }`, seeds defaults via `extractDefaultValues`, and assigns a unique `featureID` produced by `generateId()` (`src/PartHistory.js:642-654`). `sanitizeInputParams()` later evaluates expressions, resolves reference selections back into live scene objects, and normalizes booleans, vectors, transforms, or boolean-operation payloads (`src/PartHistory.js:665-758`).
