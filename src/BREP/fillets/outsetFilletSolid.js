// outsetFilletSolid.js — builds OUTSET fillet cutter solids
// Split from filletSolid.js to keep inset/outset generation separate.
// Exports:
//   - outsetFilletSolid(targetSolid, edgeName, radius, inflate = 0.1, options = {})
//   - OutsetFilletSolid (class)
...
/* --------------------------- Public Function ------------------------- */
export function outsetFilletSolid(targetSolid, edgeName, radius, inflate = 0.1, options = {}) {
  const r = +radius;
  if (!(r > 0)) throw new Error("outsetFilletSolid: radius must be > 0");
  const inflateDist = +inflate;
  if (!(inflateDist >= 0)) throw new Error("outsetFilletSolid: inflate must be >= 0");
  ...
  if (!edgeData.polyline || edgeData.polyline.length < 2) throw new Error("outsetFilletSolid: edge polyline too short");
  ...
  let seamInsetAbs = tol.epsW; // OUTSET can use tiny or zero, but keep small bias for robustness
  try {
  ...
  try {
    ...
    if (!sideA.length || !sideB.length) throw new Error("outsetFilletSolid: failed to classify band sides");
    ...
    if (rings.length < 2) throw new Error('outsetFilletSolid: insufficient valid sections');
    ...
    const booleanType = 'UNION'; // OUTSET always unions
    ...
    const toolSolid = Adapter.newSolidLike(targetSolid, toolMesh.vertices, toolMesh.faces);
    return { toolSolid, booleanType: 'UNION' };
  }
}

export class OutsetFilletSolid extends BetterSolid {
  constructor({ edgeToFillet, radius = 1, debug = false, inflate = 0.1 } = {}) {
    super();
    if (!edgeToFillet) throw new Error('OutsetFilletSolid: edgeToFillet is required');
    ...
    if (!tgt) throw new Error('OutsetFilletSolid: edge must be part of a solid');
    const { toolSolid } = outsetFilletSolid(tgt, this.edgeToFillet, this.radius, this.inflate, {});
    new CopyFrom(toolSolid, this);
    this.filletType = 'UNION';
